...
\title{A new sequential accident model to classify single-cyclist crashes}
...
%
On one hand, the expression single-bicycle crash, even being the most common, neglects the rider, making reference to a problem only oriented on the bicycle.
%
...
When the bicycle is modelled as a linear system, has the property of \textit{controllability}, defined in control theory as the ability to modify the system's states from control actions in a finite time interval \citep{Sef01, Ron24}.
%
...
%
Therefore, accelerations of the system are a key indicator to assess the probability of a crash.
%%
...
A single-cyclist crash is the result of the rider losing control of the bicycle, which is not being able to maintain stability or follow the desired trajectory.
%
\cite{Ful05} describes in his car-crash model the loss of control as a relation between task demand and rider capability.
%
...
%
As previously shown, bicycle's rotations in a three-dimensional space are fundamental to explain crash motions.
%
...
\toprule
\textbf{Mechanism} & \textbf{Description} & \textbf{Discussed in} \\
\midrule
\endfirsthead
...
Braking mistakes & brake over-actuation can produce wheel lock, which can end in a pitch-over crash or losing grip in one of the tyres. & \cite{Alg22, Gil21, Her18, Ohl19, Sch12} \\
...
Other perturbations & external forces that can not be attributed to infrastructure or rider factors. & \cite{Sch12} \\
...
Skidding & one or both tyres suddenly loss grip. & \cite{Alg22, Bec19, Her18, Ohl19, Sch12, Utr20} \\
...
%
Depending on the human control and the characteristics of the signal, the task demand increases above rider capability limit, which causes one or more crash mechanisms to appear.
%
From the crash mechanisms, it is possible to identify its location, which causes one of the characteristic motions, finally ending in a crash.
%%
...
Low speed riding & mainly referred to start and stop riding, where users require major balance actions to maintain the bicycle in an upright position. \\
Stunting & any action that is not intended for riding, such as riding with one hand, no hands, or standing on the pedals, which changes and limits the control of the rider on the bicycle. \\
...
Contact forces & this makes reference to any collision or force exerted on the bicycle, such as collision with an object, or steering perturbations. \\
...
Rider-bicycle joint & the rider is ‘attached’ to the bicycle in five points: handlebar (both hands), pedals and saddle. When one or more of these detaches, the control from the rider changes and it is prone to fail. \\
...
%
Note that motorcycle crash analyses are referenced to explain this crash motions, since the available bicycle-related literature is scarce and does not provide a detailed description, see Table \ref{tab: motion}. %of the events.
...
\begin{itemize}
    ...
    %
    Through these factors, the rider understands their own state and the state of the bicycle.
    %
